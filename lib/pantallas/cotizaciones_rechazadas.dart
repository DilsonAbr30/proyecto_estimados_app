...
class CotizacionesRechazadasScreen extends StatefulWidget {
  const CotizacionesRechazadasScreen({super.key});

  @override
  State<CotizacionesRechazadasScreen> createState() =>
      _CotizacionesRechazadasScreenState();
}

class _CotizacionesRechazadasScreenState
    extends State<CotizacionesRechazadasScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        stream: _firestore
            ...
            .where('estado_estimado', whereIn: ['rechazada', 'rechazada_cliente']) // <-- Filtro cambiado a 'whereIn'
            .orderBy('fechaCreacion', descending: true)
            .snapshots(),
        // --- FIN DE LA ACTUALIZACIÓN ---
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            // Este error SÍ puede pasar si no tienes el índice
            // pero lo más probable es que el índice que ya creaste funcione.
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => DetallesCotizacionAceptadaScreen( // Reutiliza la de "aceptada" (solo lectura)
                          cotizacionId: doc.id,
                          cotizacionData: data,
                        ),
                      ),
